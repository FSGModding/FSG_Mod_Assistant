...
class modFileChecker {
	...
	async getInfo() {
		...
		try {
			...
			if ( ! this.#util_openFile() ) {
				throw 'Unable to Open File'
			}

			if ( this.#util_fileExists('careerSavegame.xml')) {
			...
			this.modDescRAW = this.#util_textFileRead('modDesc.xml')

			if ( this.modDescRAW === null ) {
				this.#util_raiseFlag_broken = 'NOT_MOD_MODDESC_MISSING'
				this.md5Sum                 = null
				this.#log.notice(`Mod file missing modDesc.xml: ${this.uuid}`)
			...
			if ( this.modDesc.mapConfigFile !== null ) {
				try {
					const mapConfigFile = this.#util_textFileRead(this.modDesc.mapConfigFile)

					if ( mapConfigFile === null ) { throw 'Config file does not Exist'}

					const cropInfo = new cropDataReader(
						...this.#doStep_parseMapXML(this.modDesc.mapConfigFile, mapConfigFile).map((x) => this.#util_textFileRead(x))
					)
			...
			try {
				if ( this.#flag_problem.MOD_ERROR_NO_MOD_ICON || typeof this.modDesc.iconFileName !== 'string' || ! this.#util_fileExists(this.modDesc.iconFileName) ) {
					throw 'File does not Exist'
				}

				this.modDesc.iconImageCache = this.#iconParser.parseDDS(
					this.#util_binFileRead(this.modDesc.iconFileName),
					false
				)
			} catch (e) {
	...
	#doStep_fileCounts() {
		const fileList = this.fileDetail.isFolder ?
			globSync('**', { cwd : this.fileDetail.fullPath, follow : true, mark : true, stat : true, withFileTypes : true }) :
	...
	#modDesc_localString(key, fallback = '') {
		const searchTree = this.modDescParsed?.moddesc?.[key.toLowerCase()] ?? fallback
	...
	#doStep_parseModDesc() {
		if ( this.modDescRAW === false ) { return }

		this.modDescParsed = this.#util_parseXML('modDesc.xml', this.modDescRAW, 'moddesc')
	...
	#doStep_parseMapXML(fileName, fileContents) {
		const mapConfigParsed  = this.#util_parseXML(fileName, fileContents, 'map')
	...
	#util_raiseFlag(flag, flagSet) { this[`#flag_${flagSet}`][flag] = true }

	set #util_raiseFlag_problem(flag) { this.#util_raiseFlag(flag, 'problem') }
	set #util_raiseFlag_broken(flag)  { this.#util_raiseFlag(flag, 'broken') }
	set #util_raiseFlag_info(flag)    { this.#util_raiseFlag(flag, 'info') }
	...
	#util_parseXML(fileName, rawXML, defaultKey = null) {
		...
		const thisXMLParser = new XMLParser({
			...
			ignorePiTags           : true,
			isArray                : (_, jPath) => { return alwaysArray.has(jPath) },
		...
		try {
			const thisParsedXML = (defaultKey === null ? thisXMLParser.parse(rawXML) : thisXMLParser.parse(rawXML)?.[defaultKey] ) ?? null
	...
	get #util_openFile() {
		if ( ! this.fileDetail.isFolder ) {
			try {
				this.#ZIPFile = new admZip(this.fileDetail.fullPath)
			} catch (e) {
				this.#util_raiseFlag_broken = 'FILE_ERROR_UNREADABLE_ZIP'
				this.#log.warning(`Zip file failure: ${e}`)
				return false
			}
		}
		return true
	...
	#util_closeFile() {
		if ( ! this.fileDetail.isFolder ) { this.#ZIPFile = null }
	}

	#util_fileExists(fileName) {
		if ( fileName === null ) { return false }
		if ( this.fileDetail.isFolder ) {
			return fs.existsSync(path.join(this.fileDetail.fullPath, fileName))
		}
		return ( this.#ZIPFile.getEntry(fileName) === null )
	}

	#util_textFileRead(fileName) { return this.#util_fileRead(fileName, true) }
	#util_binFileRead(fileName) { return this.#util_fileRead(fileName, false) }

	#util_fileRead(fileName, text = true) {
		if ( ! this.#util_fileExists(fileName) ) { return null }

		try {
			if ( this.fileDetail.isFolder ) {
				return text ?
					fs.readFileSync(path.join(this.fileDetail.fullPath, fileName), 'utf8') :
					fs.readFileSync(path.join(this.fileDetail.fullPath, fileName), null).buffer
			}
			return text ? this.#ZIPFile.readAsText(fileName) : this.#ZIPFile.readFile(fileName).buffer
		} catch (err) {
			this.#log.notice(`Mod file read error ${fileName} :: ${err}`)
		}
	}
}
...
class modLooker {
	#iconParser     = maIPC.decode
	#fullFileName   = null
	#locale         = maIPC.l10n.deferCurrentLocale
	#log            = null
	#l10n           = null
	#modIsFolder    = true
	#modRecord      = null
	...
	async getInfo() {
		if ( ! fs.existsSync(this.#fullFileName) ) { return null }
	...
	#getXMLFile(name) {
		...
		try {
			...
			this.#log.notice(`File Read Failure: ${name} :: ${e}`)
		}
		...
		if ( fileContents !== null ) {
			try {
				...
				this.#log.warning(`File Parse Failure: ${name} :: ${err}`)
			}
		}
	...
	#getCategory(category) {
		...
		if ( typeof this.#catDescMap[category] !== 'undefined' ) {
			return this.#translate_single(this.#catDescMap[category])
		}
		return category
	}
	...
	#parseMotor(xml) {
		...
		for ( let i = 0; i < xml.length; i++ ) {
			...
			lastTorque = thisMotor?.motor?.torque ?? lastTorque
			const torqueDef = lastTorque

			lastTransmission = thisMotor?.transmission ?? lastTransmission
			const thisTrans = lastTransmission

			const axelRatio  = thisTrans?.$?.AXLERATIO ?? 1
			const minFwdGear = thisTrans?.$?.MINFORWARDGEARRATIO ?? null
			const motorScale = thisMotor?.motor?.$?.TORQUESCALE ?? 1
			const motorRPM   = thisMotor?.motor?.$?.MAXRPM ?? lastMaxRPM ?? 1800
			...
			const names = [
				this.#translate(thisMotor?.$?.NAME) ?? null,
				this.#translate(thisTrans?.$?.NAME) ?? null,
			...
			if ( minFwdGear === null ) {
				if ( typeof thisTrans?.forwardgear === 'object' ) {
					for ( const thisGear of thisTrans.forwardgear) {
						let   thisGear_ratio = thisGear?.$?.GEARRATIO ?? null
						const thisGear_max   = thisGear?.$?.MAXSPEED ?? null
			...
			for ( let j = 0; j < torqueDef.length; j++ ) {
				...
				const thisScale  = thisEntry?.$?.NORMRPM ?? 1
				const thisTorque = thisEntry?.$?.TORQUE ?? 1
				const thisRPM    = thisEntry?.$?.RPM ?? thisScale * motorRPM
				const thisHP     = motorScale*(1.35962161*Math.PI*thisRPM*thisTorque)/30
				const thisSpeed  = 3.6*((thisRPM*Math.PI)/(30*minGear))

				theseHP.push({x : Math.round(thisRPM), y : Math.round(thisHP)})
				theseKPH.push({x : Math.round(thisRPM), y : Math.round(thisSpeed)})
				theseMPH.push({x : Math.round(thisRPM), y : Math.round(thisSpeed * 0.621371)})
			}
			...
			if ( matchIndex > -1 ) {
				...
				motorInfo.hp.push({
					backgroundColor        : `${this.#getColor(i)}88`,
					borderColor            : this.#getColor(i),
					cubicInterpolationMode : 'monotone',
					data                   : theseHP,
					label                  : [[...names].filter((x) => x !== null).join(' - ')],
					pointHoverRadius       : 12,
					pointRadius            : 8,
					pointStyle             : 'circle',
					tension                : 0.4,
