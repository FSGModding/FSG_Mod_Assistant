...
class modFileChecker {
		...
		try {
			...
			if ( this.modDesc.mapConfigFile !== null ) {
				try {
					...
					const cropConfigFiles = this.#doStep_parseMapXML(this.modDesc.mapConfigFile)

					const cropInfo = new cropDataReader(
						this.#modHandle.readText(cropConfigFiles[0]),
						this.#modHandle.readText(cropConfigFiles[1]),
						this.#modHandle.readText(cropConfigFiles[2]),
						cropConfigFiles[3]
					)
	...
	get #doStep_badges() {
		const badges = {
			...
			malware : this.#flag_broken.MALICIOUS_CODE,
	...
	#util_checkLUA(fileName) {
		if ( malwareFalse.has(this.fileDetail.shortName)) { return }

		const luaContents = this.#modHandle.readText(fileName)

		if ( luaContents.match(/\.deleteFolder/) ) {
			this.#util_raiseFlag_broken('MALICIOUS_CODE')
		} else if ( luaContents.match(/\.deleteFile/) ) {
			this.#util_raiseFlag_broken('MALICIOUS_CODE')
		}
		// console.log(luaContents)
	}
	#util_countFile(suffix, fileName, size, fullFileName) {
		...
		switch (shortSuffix) {
			...
			case 'lua' :
				...
				this.#util_checkLUA(fullFileName)
				break
			case 'cache' :
