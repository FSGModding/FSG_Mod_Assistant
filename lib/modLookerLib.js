	...
	#processIcon(buffer) {
		...
		try {
			...
			if ( ddsData.format === 'dxt10' && this.#converter !== null && this.#tempFolder !== null ) {
				...
				cp.execSync(`"${this.#converter}" "${path.join(realFolder, tempName)}.dds" -ft png -srgb -o "${realFolder}"`)

				imageReturn = `data:image/png;base64, ${fs.readFileSync(path.join(realFolder, `${tempName}.png`), 'base64')}`

				fs.rmSync(path.join(realFolder, `${tempName}.dds`))
				fs.rmSync(path.join(realFolder, `${tempName}.png`))
