...
class modLooker {
	...
	#processIcon(buffer) {
		...
		try {
			let ddsData   = parseDDS(theBuffer)
			...
			if ( ddsData.format === 'dxt10' && this.#converter !== null && this.#tempFolder !== null ) {
				...
				cp.execSync(`"${this.#converter}" "${path.join(realFolder, tempName)}.dds" -pow2 -px new_ -f DXT5 -srgb -o "${realFolder}"`)

				theBuffer = fs.readFileSync(path.join(realFolder, `new_${tempName}.dds`)).buffer
				ddsData   = parseDDS(theBuffer)

				fs.rmSync(path.join(realFolder, `${tempName}.dds`))
				fs.rmSync(path.join(realFolder, `new_${tempName}.dds`))
			}
			...
			const imageDataView = new DataView(theBuffer, image.offset, image.length)
