...
class modReader {
	...
	async #readFiles() {
		...
		try {
			modFolderFiles.forEach((thisFile) =>{
				if ( thisFile.isSymbolicLink() ) {
					try {
						...
						const thisSymLink = fs.readlinkSync(path.join(this.modFolder, thisFile.name))
						const thisSymLinkStat = fs.lstatSync(path.join(this.modFolder, thisSymLink))
						...
						this.fullList[shortName] = new modFile(
							...
							this.locale,
							thisSymLinkStat.isDirectory()
						)

						this.fullList[shortName].date     = thisSymLinkStat.ctime

						if ( thisSymLinkStat.isDirectory() ) {
							let bytes = 0
							...
							this.fullList[shortName].fileSize = thisSymLinkStat.size
						}
					} catch (addError) {
			...
			this.log.fileError('fileReader', `Symlink Module Error : ${addError}`)
		}
			...
			zip.stream('modDesc.xml', (err, stm) => {
				if ( err ) {
					...
					stm.on('end', () => {
						...
						if ( this.#modI3DFiles.length < shaderCheckMaxi3Ds ) {
		...
		i3dXMLParser.parseString(fileContents, (err, result) => {
			...
			if ( this.#nestedXMLProperty('i3d.files', result) ) {
				try {
					...
					this.#modList.log.fileError(this.shortName, `Soft-Ignore: Could not parse i3d: ${err}`)
				}
			}
		})
	}
