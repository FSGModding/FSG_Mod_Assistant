...
class modFile {
		...
		if ( !this.#isFolder && !this.#fail.garbage_file ) {
			if ( this.#md5cache === null ) {
				...
				try {
					...
					this.#modList.log(this.shortName, `MD5 sum (update) error: ${err}`)
				}

				try {
					this.#md5cache = sum.digest('hex')
				} catch (err) {
					this.#modList.log(this.shortName, `MD5 sum (hash) error: ${err}`)
				}
			}
	...
	#processIcon(buffer) {
		...
		try {
			...
			this.log.icon(this.shortName, `Unknown icon processing error: ${err}`)
			return true
		}
	}
