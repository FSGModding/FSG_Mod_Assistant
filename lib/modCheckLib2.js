...
class modFileChecker {
	...
	#processModDesc() {
		...
		this.modDesc.mapConfigFile = this.modDesc.xmlParsed?.moddesc?.maps?.map?.[0]?.$?.CONFIGFILENAME ?? null
		...
		/* Get icon filename */
		let iconFileName = this.modDesc.xmlParsed?.moddesc?.iconfilename ?? false
	...
	#processMapConfig(fileContents) {
		...
		try {
			...
			this.#log.log.warning(`Caught unrecoverable Map XML Parse error: ${err}`, this.#logUUID)
