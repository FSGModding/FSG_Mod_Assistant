...
class modFileChecker {
	...
	#getLocalString(key) {
		const searchTree = this.modDesc.xmlParsed?.moddesc?.[key.toLowerCase()]?.[0] ?? null
		let   foundTerm  = null

		if ( searchTree === null ) { return null }

		try {
			foundTerm ??= searchTree?.[this.#locale()]?.[0]?.trim?.() ?? null
			foundTerm ??= searchTree?.en?.[0]?.trim?.() ?? null
			foundTerm ??= searchTree?.de?.[0]?.trim?.() ?? null
		} catch (err) {
	...
	#processModDesc() {
		...
		this.modDesc.descVersion        = this.modDesc.xmlParsed?.moddesc?.$?.DESCVERSION ?? 0
		this.#failFlags.bad_modDesc_ver = ( this.modDesc.descVersion === 0 )
	
		/* Get MOD Version */
		this.modDesc.version      = this.modDesc.xmlParsed?.moddesc?.version?.toString?.() ?? '0.0.0.0'
		this.#failFlags.no_modVer = ( this.modDesc.version === '0.0.0.0' )

		this.modDesc.author        = this.modDesc.xmlParsed?.moddesc?.author ?? 'n/a'
		this.modDesc.multiPlayer   = this.modDesc.xmlParsed?.moddesc?.multiplayer?.$?.SUPPORTED ?? false
		this.modDesc.storeItems    = this.modDesc.xmlParsed?.moddesc?.storeitems?.storeitem?.length ?? 0
		this.modDesc.mapConfigFile = this.modDesc.xmlParsed?.moddesc?.maps?.map?.[0]?.$?.CONFIGFILENAME ?? false
		this.modDesc.depend        = this.modDesc.xmlParsed?.moddesc?.dependencies?.dependency ?? []
		
		this.#failFlags.might_be_crack = typeof ( this.modDesc.xmlParsed?.moddesc?.productid ) !== 'undefined'

		/* Get icon filename */
		let iconFileName = this.modDesc.xmlParsed?.moddesc?.iconfilename ?? null

		iconFileName = ( !iconFileName === null && ! iconFileName.endsWith('.dds') ) ? `${iconFileName.slice(0, -4)}.dds` : iconFileName
		...
		try {
			if ( typeof this.modDesc.xmlParsed?.moddesc?.actions?.action !== 'undefined' ) {
				for ( const action of this.modDesc.xmlParsed.moddesc.actions.action ) {
					this.modDesc.actions[action.$.NAME] = action.$.CATEGORY || 'ALL'
				}
			}
			if ( typeof this.modDesc.xmlParsed?.moddesc?.inputbinding?.actionbinding !== 'undefined' ) {
				for ( const action of this.modDesc.xmlParsed.moddesc.inputbinding.actionbinding ) {
