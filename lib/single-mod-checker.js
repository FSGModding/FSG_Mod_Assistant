...
class modFileChecker {
	...
	#processModDesc() {
		...
		strictXMLParser.parseString(this.modDesc.xmlDoc, (err, result) => {
			if ( err !== null ) {
				...
				this.#log.fileError(this.fileDetails.shortName, `Caught XML Parse error: ${err}`)
				...
				looseXMLParser.parseString(this.modDesc.xmlDoc, (err, result) => {
					if ( err !== null ) {
						/* Couldn't recover */
						this.#log.fileError(this.fileDetails.shortName, `Caught unrecoverable XML Parse error: ${err}`)
	...
	#testZip() {
		...
		zipEntries.forEach((entry) => {
			...
			if ( entry.entryName.endsWith('.l64') || entry.entryName.endsWith('productID.dat')) {
				this.#failFlags.might_by_crack = true
			}
			if ( entry.entryName.endsWith('.lua') ) {
				this.fileDetail.scriptFiles++
			}
		...
		try {
			...
			this.#log.fileError(this.fileDetails.shortName, `Zip missing file: modDesc.xml: ${e}`)
		}
	...
	#testFolder() {
		...
		for ( const checkFile of allFileList ) {
			...
			if ( checkFile.endsWith('.lua') ) { this.fileDetail.scriptFiles++ }
