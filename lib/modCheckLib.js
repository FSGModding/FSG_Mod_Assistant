...
class modFileCollection {
			...
			switch (m.type) {
				...
				case 'modRecord' :
					...
					if ( m.modRecord.md5Sum !== null && !m.modRecord.canNotUse ) {
						...
						this.#modCacheStore[m.modRecord.md5Sum] = m.modRecord
					}
					break
				default :
					break
	...
	#doModBadge(modRecord, hasConflicts = false, hasDepend = false) {
		if ( modRecord.fileDetail.isSaveGame ) {
			return new badgeHandle(['notmod', 'savegame'], this.#badgeNamed).keyList
		}
...
class badgeHandle {
	...
	get keyList() {
		const sorted = Array.from(this.#baseSet).sort((a, b) => Intl.Collator().compare(this.#l10nNames[a], this.#l10nNames[b]))
		const returnArray = []
