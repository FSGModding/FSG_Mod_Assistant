...
class modFileChecker {
	...
	#processModDesc() {
		...
		try {
			...
			this.#log.warning('Caught unrecoverable XML Parse error')
		...
		if ( parsedDesc === null ) {
			this.#log.warning('ModDesc XML is not formed correctly')
			this.#failFlags.bad_modDesc_no_rec = true
			return
		}
		...
		if ( typeof parsedDesc?.iconfilename ==='string' ) {
			iconFileName = parsedDesc.iconfilename
		} else if ( typeof parsedDesc?.iconfilename?.[0] ==='string' ) {
			iconFileName = parsedDesc.iconfilename[0]
		}
	...
	#processMapConfig(fileContents) {
		...
		try {
			...
			this.#log.warning('Caught unrecoverable Map XML Parse error')
	...
	async #testZip() {
		...
		try {
			if ( zipFile.getEntry('modDesc.xml') === null ) {
				if ( zipFile.getEntry('careerSavegame.xml')  !== null ) {
					...
					this.#log.notice('Zip file is a savegame archive')
			...
			this.#log.notice(`Zip file missing modDesc.xml: ${e}`)
		}
		...
		try {
			if ( typeof this.modDesc.iconFileName !== 'string' || zipFile.getEntry(this.modDesc.iconFileName) === null ) {
				throw 'File does not Exist'
			}
	...
	async #testFolder() {
		...
		try {
			...
			this.#log.warning(`Couldn't read modDesc.xml: ${e}`)
		...
		try {
			if ( typeof this.modDesc.iconFileName !== 'string' || !fs.existsSync(path.join(this.fileDetail.fullPath, this.modDesc.iconFileName)) ) {
				throw 'File does not Exist'
			}
			this.modDesc.iconImageCache = this.#iconParser.parseDDS(
...
class modLooker {
	...
	#parseMotor(xml) {
		...
		for ( let i = 0; i < xml.length; i++ ) {
			...
			if ( typeof thisMotor?.motor?.torque !== 'undefined' ) {
				lastTorque = thisMotor.motor.torque
			}

			const torqueDef = lastTorque


			if ( typeof thisMotor?.transmission !== 'undefined' ) {
				lastTransmission = thisMotor.transmission
			}
