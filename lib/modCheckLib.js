...
class modFileCollection {
	...
	constructor(homeDir, emitter, skipCache = false, noBot = false) {
		this.#userHome            = homeDir
		this.#skipCache           = skipCache
		this.#threadDoneEmit      = emitter
		this.#botDisabled         = noBot
	}
	...
	#botRunner() {
		...
		if ( thisRun > this.#botLastRun + this.#botMinInterval ) {
			this.#log.log.info('Updating FSG Bot Information', 'fsg-bot-poll')
			if ( this.#botRequestList.size === 0 ) {
				this.#log.log.debug('Bot request list empty, stopping interval', 'fsg-bot-poll')
				if ( this.#botInterval !== null ) {
			...
			if ( net.isOnline() ) {
				...
				request.on('response', (response) => {
					this.#log.log.debug(`Got FSG Bot response: ${response.statusCode}`, 'fsg-bot-poll')
					let responseData = ''
		
					response.on('error', (err) => {
						this.#log.log.info(`Network error : ${thisURL} :: ${err}`, 'fsg-bot-poll')
					})
					...
					response.on('end',  () => {
						try {
							...
							this.#log.log.info(`File error :: ${err}`, 'fsg-bot-poll')
						}
						
					})
				})
				request.on('abort', () => {
					this.#log.log.info('Network abort', 'fsg-bot-poll')
				})
				request.on('error', (err) => {
					this.#log.log.info(`Network error :: ${err}`, 'fsg-bot-poll')
				})
				request.end()
			}
		}
	...
	get botDetails() {
		return {
			l10nMap     : {
				online  : maIPC.l10n.syncStringLookup('bot_online'),
				offline : maIPC.l10n.syncStringLookup('bot_offline'),
				unknown : maIPC.l10n.syncStringLookup('bot_unknown'),
			},
	...
	async toRenderer(extra = null) {
		// DATA STRUCT : send mod list toRenderer()
