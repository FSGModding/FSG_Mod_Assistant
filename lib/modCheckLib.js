...
class savegameTrack {
	...
	constructor(savePath) {
		...
		this.#logName  = `savegame_track_${path.basename(savePath)}`
		...
		if ( ! this.#findFiles() ) {
			this.#log.log.danger('Savegame not found, or backup list empty', this.#logName)
			return
		}
		...
		if ( currentSave === null ) {
			this.#log.log.danger('Original save could not be read', this.#logName)
			return
		}
		...
		if ( this.#saveMods.byDate.length > 0 ) {
			...
			this.#log.log.info('No backups found for savegame, useless results', this.#logName)
		}
	}
	...
	#findFiles () {
		...
		if ( this.#backups.length < 1 ) { return false }
