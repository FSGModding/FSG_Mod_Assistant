...
class modFileCollection {
	...
	constructor(logger, notes, cache, homeDir, loadingWindow, locale, skipCache = false) {
		this.#modCache            = cache
		this.#log                 = logger
	...
	modColUUIDsToRecords(IDs) {
		const theseMods = []
		IDs.forEach((thisColUUID) => { theseMods.push(this.modColUUIDToRecord(thisColUUID)) })
		return theseMods
	}
	...
	async processMods() {
		...
		this.#set_Collections.forEach((collectKey) => {
			this.#map_FolderContents[collectKey].forEach((thisFile) => {
		...
		console.log(this.#scanPromise.length)
	}
	...
	#fileStats(collectKey, thisFile) {
		...
		try {
			...
			if ( fileStats.isFolder ) {
				let bytes = 0
				glob.sync('**', { cwd : path.join(fullPath) }).forEach((file) => {
					try {
	...
	#addModToData(collectKey, modRecord) {
		modRecord.currentCollection = collectKey
		modRecord.colUUID           = `${collectKey}--${modRecord.uuid}`
		modRecord.modHub            = this.modHubFullRecord(modRecord, false)

		this.#map_ModUUIDToShortName[modRecord.uuid] = modRecord.fileDetail.shortName
		this.#list_allMods[collectKey].mods[modRecord.uuid] = modRecord
		this.#list_allMods[collectKey].folderSize += modRecord.fileDetail.fileSize
		this.#list_allMods[collectKey].modSet.add(modRecord.uuid)
		this.#list_allMods[collectKey].dependSet.add(modRecord.fileDetail.shortName)
		this.#list_allMods[collectKey].alphaSort.push(`${modRecord.fileDetail.shortName}::${modRecord.uuid}`)
	}
	...
	#processBindConflicts() {
		...
		this.#set_Collections.forEach((collectKey) => {
			...
			this.#list_allMods[collectKey].modSet.forEach((modUUID) => {
		...
		Object.keys(this.#bindConflict).forEach((collectKey) => {
			Object.keys(this.#bindConflict[collectKey]).forEach((modName) => {
	...
	async #addMod(collectKey, thisFile) {
		...
		try {
			...
			if ( thisFileStats.hashString !== null ) {
			...
			const thisBadMod = new notModFileChecker(
				thisFileStats.fullPath,
				false,
				thisFileStats.size,
				thisFileStats.date,
			...
			this.#addModToData(collectKey, thisBadMod)
			this.#loadingWindow.count()
		}
	}

	async toRenderer(extra = null) {
		this.#log.log.debug('Collection Render Return Called', 'collection-reader')

		return Promise.all(this.#scanPromise).then(() => {
			this.#log.log.debug('Collection Render Return Firing', 'collection-reader')
			...
			return {
				...
				collectionNotes            : this.#map_CollectionNotes.store,
	...
	#processModDesc() {
		...
		try {
			if ( this.#nestedXMLProperty('moddesc.dependencies') ) {
				this.modDesc.xmlParsed.moddesc.dependencies[0].dependency.forEach((dep) => {
		...
		try {
			if ( this.#nestedXMLProperty('moddesc.actions') ) {
				this.modDesc.xmlParsed.moddesc.actions[0].action.forEach((action) => {
					this.modDesc.actions[action.$.NAME] = action.$.CATEGORY || 'ALL'
				})
			}
			if ( this.#nestedXMLProperty('moddesc.inputbinding')) {
				this.modDesc.xmlParsed.moddesc.inputbinding[0].actionbinding.forEach((action) => {
