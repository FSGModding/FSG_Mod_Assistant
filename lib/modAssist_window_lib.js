...
const { screen, app, nativeTheme, dialog, shell, BrowserWindow, nativeImage } = require('electron')
...
module.exports.windowLib = class {
	...
	winDefs = {
		...
		change : {
			...
			subWindowArgs   : { parent : 'main', fixed : true, preload : 'confirmCombi' },
			callback        : () => { return },
			handleURLinWin  : true,
		},
		debug : {
	...
	#getNativeImage(icon) {
		...
		return new nativeImage.createFromPath(path.join(basePath, `${icon}.png`))
	}
	...
	populateContextIcons() {
		...
		this.contextIcons.modhub          = this.#getNativeImage('ui-icon-modhub')
