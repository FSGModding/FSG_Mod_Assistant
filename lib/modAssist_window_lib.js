...
const { screen, app, nativeTheme } = require('electron')
...
module.exports.windowLib = class {
	...
	#verifyWindow(name) {
		return ( typeof self.win[name] !== 'undefined' && self.win[name] !== null && !self.win[name].isDestroyed() )
	}
	#visibleWindow(name) {
		return this.win[name] !== null && this.win[name].isVisible()
	}
	#sendToWindow(name, command, ...args) {
		self.win[name].webContents.send(command, ...args)
	}

	set settings(settingsLib) { this.#settings = settingsLib }
	...
	themeUpdater() {
		for ( const thisWinKey in this.win ) {
			if ( this.#visibleWindow(thisWinKey) ) {
	...
	toggleMainDangerFlag() {
		if ( this.#verifyWindow('main') ) { this.#sendToWindow('main', 'fromMain_debugLogDanger') }
	}
}
