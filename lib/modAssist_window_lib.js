...
module.exports.windowLib = class {
	...
	winDefs = {
		...
		setup : {
			...
			callback        : () => {},
			// callback        : (windowArgs) => { this.sendModList(windowArgs, 'fromMain_collectionName', 'setup', false ) },
			refocusCallback : true,
			handleURLinWin  : true,
			extraCloseFunc  : () => { this.#refFunc.refreshClientModList() },
		},
	...
	#getNativeImage(icon) {
		...
		const isDark = nativeTheme.shouldUseDarkColors ? '' : 'blk-'
