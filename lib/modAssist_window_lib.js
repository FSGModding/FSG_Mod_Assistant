...
module.exports.windowLib = class {
	...
	winDefs = {
		...
		folder : {
			...
			subWindowArgs   : { parent : 'main', fixed : true,  preload : 'folderWindow' },
		...
		mini : {
			...
			callback        : () => { this.#refFunc.refreshClientModList() },
			refocusCallback : true,
		...
		prefs : {
			winName         : 'prefs',
			HTMLFile        : 'prefs.html',
			subWindowArgs   : { preload : 'prefsWindow' },
			callback        : () => {
	...
	sendModList(extraArgs = {}, eventName = 'fromMain_modList', toWindow = 'main', closeLoader = true) {
		this.#collect.toRenderer(extraArgs).then((modCollection) => {
			...
			if ( closeLoader && !maIPC.processing ) { this.loading.hide(1500) }
		})
	}
	...
	resetPositions() {
		...
		try {
			...
			this.#log.log.debug(`Reset failed : ${err}`, 'reset-windows')
		}
	...
	toggleMainDangerFlag() {
		this.sendToValidWindow('main', 'fromMain_debugLogDanger')
	}
	...
	createNamedWindow(winName, windowArgs, extraCallback = null) {
		...
		this.win[thisWinName].webContents.on('did-finish-load', async () => {
			// TODO - FONT SCALING!!
			//this.win[thisWinName].webContents.insertCSS(':root { --bs-root-font-size: 24px !important; }', { cssOrigin : 'user'})
	...
	createMainWindow (openCallback) {
		...
		this.win.main.webContents.on('did-finish-load', () => {
			// TODO - FONT SCALING!!
			//this.win.main.webContents.insertCSS(':root { --bs-root-font-size: 24px !important; }', { cssOrigin : 'user'})
