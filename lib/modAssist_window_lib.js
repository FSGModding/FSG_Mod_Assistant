...
module.exports.windowLib = class {
	...
	winDefs = {
		...
		prefs : {
			...
			callback        : () => {
				this.sendToValidWindow('prefs', 'fromMain_allSettings', this.#settings, this.#flags.devControls )
			},
			refocusCallback : true,
			handleURLinWin  : true,
		...
		looker : {
			...
			callback        : (windowArgs) => { this.sendModList(windowArgs, 'fromMain_modRecord', 'looker', false) },
			refocusCallback : true,
			handleURLinWin  : true,
		},
		find : {
	...
	constructor(log, translator, refFunc = {}, flags) {
		this.l10nLib  = translator
		this.#flags   = flags
		this.#refFunc = refFunc
		this.#log     = log
		this.#debug   = !app.isPackaged

		this.#path.render  = path.join(app.getAppPath(), 'renderer')
		this.#path.preload = path.join(this.#path.render, 'preload')
		this.#path.icon    = path.join(app.getAppPath(), 'build', 'icon.ico')
	...
	destroyAndFocus (name) {
		this.win[name] = null
		if ( this.isValid('main') && !this.isVisible('load') ) {
			this.win.main.focus()
		}
	}
	...
	sendModList(extraArgs = {}, eventName = 'fromMain_modList', toWindow = 'main', closeLoader = true) {
		this.#collect.toRenderer(extraArgs).then((modCollection) => {
			...
			if ( closeLoader ) { this.loading.hide(1500) }
		})
	}

	set settings(settingsLib) { this.#settings = settingsLib; this.changeTheme() }
	set modCollect(collect)   { this.#collect = collect }
	...
	doDialogBox(attachTo, {type = 'info', message = null, messageL10n = null, title = null, titleL10n = null }) {
		...
		const thisTitle   = ( title !== null ) ? title : this.l10nLib.syncStringLookup(( titleL10n === null ) ? 'app_name' : titleL10n)
		const thisMessage = ( message !== null ) ? message : this.l10nLib.syncStringLookup(messageL10n)
	...
	createNamedWindow(winName, windowArgs) {
	...
	createSubWindow(winName, { useCustomTitle = true, skipTaskbar = false, noSelect = true, show = true, parent = null, title = null, fixed = false, frame = true, move = true, preload = null, fixedOnTop = true} = {}) {
		...
		const winTitle = ( title === null ) ? this.l10nLib.syncStringLookup('app_name') : title
	...
	createMainWindow (openCallback) {
		this.win.load = this.createSubWindow('load', {
			fixed          : true,
			fixedOnTop     : false,
			frame          : false,
			move           : false,
		...
		this.win.load.loadFile(path.join(this.#path.render, 'loading.html'))
		this.win.load.on('close', (event) => { event.preventDefault() })
	
		this.win.main = this.createSubWindow('main', { noSelect : false, show : this.#debug, preload : 'mainWindow' })
		
		this.win.main.on('closed', () => {
			this.win.main = null
			this.win.load.destroy()
			app.quit()
		})
		...
		this.win.main.webContents.on('did-finish-load', () => {
			const showCount = setInterval(() => {
				if ( this.isVisible('main') ) {
					...
					openCallback()
					if ( this.#debug ) { this.win.main.webContents.openDevTools() }
				}
			}, 250)
		})
...
class loadingWindow {
	...
	get isReady () { return this.#win.isVisible('load') }
	...
	#doCount (whichCount, amount, reset, inMB) {
		...
		this.#win.sendToValidWindow('load', `fromMain_loading_${whichCount}`, this[thisCounter], inMB)
	}
	
	hide (time = 1250) {
		setTimeout(() => {
			if ( this.#win.isValid('main') ) { this.#win.win.main.setProgressBar(-1)}
			if ( this.#win.isValid('load') ) { this.#win.win.load.hide() }
		}, time)
	}

	noCount () {
		this.#win.sendToValidWindow('load', 'fromMain_loadingNoCount')
	}

	open ( l10n, isDownload = false ) {
		const newCenter   = this.#win.getRealCenter('load')
		const winTitle    = this.#win.l10nLib.syncStringLookup((l10n) !== 'launch' ? `loading_${l10n}_title` : 'app_name')
		const winSubTitle = this.#win.l10nLib.syncStringLookup((l10n) !== 'launch' ? `loading_${l10n}_subtitle` : 'launch_game')
		const dlCancel    = this.#win.l10nLib.syncStringLookup('cancel_download')

		if ( this.#win.isValid('load') ) {
			try {
				this.#win.win.load.setBounds({x : newCenter.x, y : newCenter.y})
			} catch (e) {
				this.#win.win.load.center()
			}
	
			this.#win.win.load.show()
			this.#win.win.load.focus()
			this.#win.sendToWindow('load', 'formMain_loadingTitles', winTitle, winSubTitle, dlCancel)
	
			setTimeout(() => {
				if ( this.#win.isValid('load') ) {
					this.#win.win.load.show()
					this.#win.win.load.focus()
					if ( isDownload ) { this.#win.sendToWindow('load', 'fromMain_loadingDownload') }
				}
			}, 250)
			return
		}
