...
module.exports.windowLib = class {
	...
	trayContextMenu() {
		...
		this.tray.setContextMenu(Menu.buildFromTemplate([
			...
			funcLib.menu.sep,
			funcLib.menu.iconL10n('launch_game', () => { serveIPC.refFunc.gameLauncher() }, 'launch', launchExt ),
			{
				icon    : serveIPC.windowLib.contextIcons.depend,
				label   : this.l10n.syncStringLookup('launch_game_with'),
				submenu : serveIPC.modCollect.getCurVerCollections(funcLib.prefs.ver()).map((x) => ({
					icon  : serveIPC.windowLib.contextIcons.active,
					label : x[2],
					click : () => {
						funcLib.gameSet.change(x[0]).then(() => { funcLib.gameLauncher() })
					},
				})),
			},
