...
const general = {
	...
	pollGame : async () => {
		...
		const gameExe = `FarmingSimulator20${serveIPC.storeSet.get('game_version', 22)}Game.exe`
...
const processor = {
	...
	addFolderTracking : (folder, overrideVersion = null, json_import = false) => {
		const newVersion = overrideVersion !== null ? overrideVersion : serveIPC.storeSet.get('game_version', 22)
...
const saveManage = {
	...
	refresh : () => {
		saveManage.init()
		const thisSaveManage = new saveGameManager(prefs.verGet('game_settings'))
	...
	restore : (fullPath, newSlot) => {
		saveManage.init()
		try {
			const newSlotFull = path.join(path.dirname(prefs.verGet('game_settings')), `savegame${newSlot}`)
