...
class ddsDecoder {
	...
	async parseDDS(fileBuffer, hasAlpha = false) {
		...
		try {
			...
			return await vipsPromise.then((vReady) => {
				const newImage = vReady.Image.newFromMemory(rgbaData, imageWidth, imageHeight, 4, vReady.BandFormat.uchar)

				const webpBuffer = newImage.writeToBuffer('.webp')

				return `data:image/webp;base64, ${btoa(String.fromCharCode.apply(null, webpBuffer))}`
			}).catch((err) => {
				throw `WEBP Encode Error: ${err}`
			})
		} catch (e) {
