...
class ma_logger {
	...
	constructor( defaultProcess, app = null, fileName = null) {
	...
	#addToLog(level, process, text) {
		...
		const simpleText    = `${logTimeString} | ${level.toUpperCase().padStart(7, ' ')} | ${cleanProcess}${text}`
		const htmlText      = `<em>${logTimeString}</em> | <span class="${this.#level_to_color[level]}">${level.toUpperCase().padStart(7, ' ')}</span> | ${cleanProcess}${text}`
