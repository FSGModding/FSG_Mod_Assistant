...
class windowState {
	...
	constructor() {
		...
		window.notes_IPC.receive('settings:collection:id', async (key) => { this.gotId(key) })
		this.init()
	}
	...
	async gotId(key) {
		...
		window.notes_IPC.get(key).then(async (results) => {
			...
			for ( const [optKey, value] of Object.entries(results) ) {
				...
				this.inputValidate(optKey, element, false)
			}
		})
	}

	// MARK : init page elements
	init() {
		...
		for ( const element of MA.query('page-replace')) {
			...
			switch ( replaceType ) {
				case 'text-input':
					element.replaceWith(this.replaceTextInput(replaceKey))
					break
				case 'password-input':
					element.replaceWith(this.replacePasswordInput(replaceKey))
					break
				case 'switch-input':
					element.replaceWith(this.replaceSwitchInput(replaceKey))
					break
				default :
					break
			}
		...
		for ( const id of changeInputs ) {
			MA.byIdEventIfExists(`notes_${id}`, (e) => { this.inputSet(e) }, 'change')
		}
	
		for ( const element of MA.queryA('[name="notes_color"]') ) {
			element.addEventListener('change', () => { this.inputColor() })
		}
		MA.byIdEventIfExists('notes_notes', window.notes_IPC.inputContext, 'contextmenu')
		MA.byIdEventIfExists('notes_notes', (e) => { this.inputSet(e) }, 'blur')
		MA.byIdEventIfExists('notes_website', window.notes_IPC.inputContext, 'contextmenu')
		MA.byIdEventIfExists('notes_website', (e) => { this.inputSet(e) }, 'blur')
		MA.byIdEventIfExists('notes_website', (e) => { this.inputKey(e) }, 'keyup')
	...
	inputColor() {
		...
		if ( checkValue !== null ) {
			window.notes_IPC.set(this.collectKey, 'notes_color', checkValue)
		}
	}
	...
	inputSet(e) {
		...
		if ( formControl.getAttribute('type') === 'checkbox' ) {
			window.notes_IPC.set(this.collectKey, dataKey, formControl.checked).then((retValue) => { formControl.checked = retValue[dataKey] })
		} else if ( formControl.tagName === 'SELECT' ) {
			window.notes_IPC.set(this.collectKey, dataKey, formControl.value)
		} else {
			window.notes_IPC.set(this.collectKey, dataKey, formControl.value).then((retValue) => { formControl.value = retValue[dataKey] })
		}
	}
	...
	inputKey(e) {
		...
		if ( ( e.code === 'Enter' || e.code === 'NumpadEnter' ) ) {
			...
			this.inputValidate(dataKey, e.target, true)
		}
	}

	inputValidate(id, element, inProgress = false) {
	...
	replaceTextInput(id) {
		...
		inputElement.addEventListener('keyup', (e) => { this.inputKey(e)} )
		inputElement.addEventListener('context', window.notes_IPC.inputContext )
		inputElement.addEventListener('blur', (e) => { this.inputSet(e)} )
		return node
	}

	replacePasswordInput(id) {
		const node = document.createElement('div')
		node.innerHTML = [
			`<i18n-text class="inset-block-header" data-key="notes_title_${id}"></i18n-text>`,
			'<div class="row">',
	...
	replaceSwitchInput(id) {
		...
		inputElement.addEventListener('change', (e) => { this.inputSet(e) })
		return node
	}
}